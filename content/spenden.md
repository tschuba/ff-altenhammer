---
title: "Spenden"
draft: false
---

Die Freiwillige Feuerwehr Altenhammer ist auf Spenden und Fördermitgliedschaften angewiesen, um Ausrüstung, Fahrzeuge und Ausbildung zu finanzieren, die über die öffentliche Förderung hinausgehen. Jeder Beitrag hilft — herzlichen Dank!

## Spendenkonto

**Feuerwehrverein Altenhammer e.V.**

| | |
|---|---|
| IBAN | <!-- TODO: IBAN eintragen --> |
| BIC | <!-- TODO: BIC eintragen --> |
| Verwendungszweck | Spende FF Altenhammer |

<!-- GIROCODE-QR-PLATZHALTER: SVG wird generiert sobald IBAN bekannt ist -->

## Spendenquittung

Für Spenden ab **300 €** stellen wir gerne eine Spendenquittung aus. Bitte fülle dazu das Formular aus:

<!-- HEYFORM-PLATZHALTER: Spendenquittungs-iFrame wird eingetragen sobald HeyForm läuft -->

Das Formular wird hier eingebunden, sobald unsere Formular-Infrastruktur eingerichtet ist.

Bis dahin wende dich bitte direkt an uns:  
**E-Mail:** [ff-altenhammer@outlook.com](mailto:ff-altenhammer@outlook.com)

---

Alternativ kannst du uns auch als [Fördermitglied]({{< relref "mitmachen" >}}#foerdernd) unterstützen — mit einem festen Jahresbeitrag, der unsere Arbeit dauerhaft stärkt.
